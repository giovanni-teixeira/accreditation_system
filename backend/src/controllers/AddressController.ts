...
export class AddressController {
    ...
    @Get(':zipCode')
    @ApiOperation({ summary: 'Busca endereço por CEP/Postcode com fallback e cache' })
    @ApiParam({ name: 'zipCode', description: 'Código postal (CEP ou Postcode)' })
    @ApiQuery({ name: 'country', required: false, description: 'País (Default: Brasil)' })
    async getAddress(
        ...
        if (!address) {
            throw new NotFoundException('Endereço não encontrado nas bases disponíveis.');
        }
